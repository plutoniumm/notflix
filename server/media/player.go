...
func VideoPlayer(c *gin.Context, videosDir string) {
	...
	if err != nil {
		library.Error(err.Error(), c, http.StatusBadRequest)
		return
	}
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			library.Error("Error opening video file: "+err.Error(), c, http.StatusInternalServerError)
		}
		return
	}
	defer file.Close()
...
func serve(c *gin.Context, file *os.File, info os.FileInfo, name string) {
	...
	start, end, clen := library.Range(rng, size)
	...
	if _, err := file.Seek(start, io.SeekStart); err != nil {
		library.Error("seek error: "+err.Error(), c, http.StatusInternalServerError)
		return
	}
